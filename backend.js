...
// 🔑 Set master key globally (works for all Parse SDK versions)
Parse.masterKey = MASTER_KEY;

// ❗ No need for the old withMasterKey wrapper – remove it entirely
// All functions that need master key already pass { useMasterKey: true }
...
const Backend = {
    ...
    async getBusinessProfile() {
        try {
            const currentUser = Parse.User.current();
            if (!currentUser) return { success: false, message: "Please login first" };
            const user = await new Parse.Query(Parse.User).get(currentUser.id, { useMasterKey: true });
            return {
                id: user.id,
